...
class SckGraph:
            ...
            result.append(
                (
                    ...
                    % (
                        node_name,
                        len(node_info["in"]),
                        str(list(node_info["in"].keys())),
                        len(node_info["out"]),
                        str(list(node_info["out"].keys())),
                    ),
                )
            )
    ...
    def compute_exec_flow(self, node_names=[], graph=None):
        ...
        for nname in node_names:
            selected_nodes += [
                node_to
                for node_from, node_to in graph[GRAPH_EDGE].keys()
                if node_from == nname
            ]
        ...
        return [current_layer] + self.compute_exec_flow(
            node_names=node_names, graph=graph
        )
    ...
    def add_edge(self, node_from, node_to, is_add_node=True):
        ...
        if self._is_edge_exist(node_from, node_to):
            ...
            self.update_parent_dependencies(
                parent_nodes=self.graph[GRAPH_NODE][node_from]["in"],
                target_node=node_to,
            )
            self.update_child_dependencies(
                source_node=node_from,
                child_nodes=self.graph[GRAPH_NODE][node_to]["out"],
            )
                ...
                if isinstance(v, ListConfig):
                    for idx, item in enumerate(v):
                        self.add_edge(
                            ...
                            sck_format.convert_list_to_sckref(
                                parent_path + [k, str(idx)]
                            ),
                        )
                        self.build_graph(
                            item, parent_path=parent_path + [k, str(idx)]
                        )  # a/b/c/3
                    ...
                    if sck_format.is_scake_method(k):
                        target_obj_node_name = (
                            self._get_node_name_from_method_description(v)
                        )
                        if not target_obj_node_name:  # is None
                            target_obj_node_name = sck_format.convert_list_to_sckref(
                                parent_path
                            )
    ...
    def query(self, key=None, keys=[], default=None, live=False):
        ...
        exec_layers = self.compute_exec_flow(
            node_names=keys
        )  # [["/config/bar/a", ...], ...]
        ...
        while len(p_queue) > 0:
            ...
            for idx in range(n_item):
                ...
                res_value = OmegaConf.select(
                    self.config,
                    sck_format.convert_sckref_to_query(target),
                    default=None,
                )
                ...
                if (
                    not live
                    and GRAPH_NODE_RESOLVED_VALUE in graph_node.get(target, {})
                    or (
                        sck_format.contains_scake_class(res_value)
                        and GRAPH_NODE_RESOLVED_VALUE in graph_node.get(target, {})
                    )
                ):  # force using reference from initialized object, because the executive layers are resolved in order
                    res_value = graph_node[target][GRAPH_NODE_RESOLVED_VALUE]
                    if not result_description:
                        final_result = (
                            res_value  # already the result we want. Eg. 1, 5 "x"
                        )
                        break
                    else:
                ...
                if sck_format.is_scake_method(target):
                    ...
                    if sck_format.is_dict(res_value):
                        ...
                        func_value = {}
                        func_value[fname] = graph_node[
                            sck_format.SCK_REF_DELIMITER.join([target, fname])
                        ][
                    ...
                    if sck_format.contains_scake_class(
                        res_value
                        ...
                        init_result = self.__init_instance(
                            class_str=class_str, param_dict=param_dict
                        )
                    ...
                    if not result_description:
                        ...
                        self._apply_result_description(
                            result_description, res_dict
                        )  # assign empty dict to result
                    ...
                    if is_instance_init:
                        ...
                        for k, v in res_value.items():
                            p_queue.append(
                                {
                                    "node": sck_format.SCK_REF_DELIMITER.join(
                                        [target, k]
                                    ),
                                    "result_description": {
                                        "type": "dict",
    ...
    def __init_instance(self, class_str, param_dict):
        module_name, class_name = class_str.lstrip(sck_format.SCK_ANNO_CLASS).rsplit(
            ".", 1
        )
