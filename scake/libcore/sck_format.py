...
def convert_sckref_to_query(sckref):
    ...
    return "".join(["[%s]"%item for item in sckref.lstrip(SCK_ANNO_REF_START).split(SCK_REF_DELIMITER)])
...
def get_parent_node(node_name):
    return SCK_ANNO_REF_START+SCK_REF_DELIMITER.join((node_name.lstrip(SCK_ANNO_REF_START).split(SCK_REF_DELIMITER))[:-1])
