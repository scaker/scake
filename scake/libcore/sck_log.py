...
class SckLog(SckSingleton):
    ...
    def register(self, name=None, obj_or_class=None, delimiter=" ", is_debug=False, is_info=False, is_warning=False, is_error=False):
        """
    ...
    def log(self, *messages, cls_name=None, name=None, delimiter=" ", is_debug=False, is_info=False, is_warning=False, is_error=False):
        ...
        final_msg = "%s %s" % (prefix, msg_str) if prefix and msg_str else (msg_str if msg_str else (prefix if prefix else ""))
        if is_debug:
