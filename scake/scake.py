...
class Scake(object):
    ...
    def exec_node(
        self, node_name, conf=OmegaConf.create(), cache={}, prefer_cache=True
    ):
        """
        ...
        if isinstance(node_name, DictConfig):
            return {
                key: self.exec_node(node_name=value, conf=conf, cache=cache)
                for (key, value) in node_name.items()
            }
        elif isinstance(node_name, ListConfig):
            return [self.exec_node(item, conf=conf, cache=cache) for item in node_name]
        else:
            if isinstance(node_name, str) and sck_format.is_scake_ref(node_name):
                node_value_in_conf = conf.get(
                    sck_format.convert_sckref_to_query(node_name), None
                )
                ...
                if sck_format.is_scake_class(node_name):
                    ...
                    # $Bar: /abc/def
                    if isinstance(
                        node_value_in_conf, DictConfig
                    ) or sck_format.is_scake_ref(node_value_in_conf):
                        result = self.exec_node(
                            node_name=node_value_in_conf, conf=conf, cache=cache
                        )
                        cache[node_name] = result
                        print("Cache set:", node_name, "=>", result)
                    else:
                        raise Exception(
                            "Not handling yet! %s, %s"
                            % (node_name, str(node_value_in_conf))
                        )
                    pass
                    ...
                    """
                    # trigger function
                    target_obj = self.exec_node(
                        node_name=sck_format.get_parent_node(node_name),
                        conf=conf,
                        cache=cache,
                    )
                    func_value = self.exec_node(
                        node_name=node_value_in_conf, conf=conf, cache=cache
                    )
                    ...
                    cache[node_name] = result
                    print("Cache set:", node_name, "=>", result)
                elif type(node_value_in_conf) in (
                    int,
                    float,
                    str,
                    bool,
                    ...
                    if sck_format.contains_scake_class(node_value_in_conf):
                        ...
                        (
                            class_str,
                            param_dict,
                        ) = sck_format.extract_class_str_and_param_dict(
                            node_value_in_conf
                        )
                        param_dict = self.exec_node(
                            node_name=param_dict, conf=conf, cache=cache
                        )
                        result = self.__init_instance(
                    ...
                    raise Exception(
                        "Not handling case! node_name (%s), node_value_in_conf(%s), node_value_in_cache(%s)"
                        % (node_name, str(node_value_in_conf), str(node_value_in_cache))
                    )
                    return node_name  # ?!
                    pass
                return result
    ...
    def exec_flow(self, node_layers):
        ...
        for nlayer in node_layers:  # ['/config/bar/a', '/config/bar/e']
            for node_name in nlayer:
                result = self.exec_node(
                    node_name, conf=self._conf, cache=self._cache_flatten
                )
